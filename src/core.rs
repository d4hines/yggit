...
pub trait GitOperations {
    fn set_branch_to_commit_with_parent(
        &self,
        branch: &str,
        oid: Oid,
...
pub fn process_dag_operations<T: GitOperations>(
    git_ops: &T,
    commits: &[EnhancedCommit<Note>],
) -> Vec<(String, bool)> {
    ...
    for commit in commits {
        ...
        let success = git_ops
            .set_branch_to_commit_with_parent(branch, *id, parent_branch.as_deref())
            .is_ok();
...
mod tests {
    ...
    fn test_end_to_end_dag_workflow() {
        ...
        assert_eq!(
            parsed_commits[0].target.as_ref().unwrap().parent_branch,
            Some("bar".to_string())
        );

        assert_eq!(parsed_commits[1].target.as_ref().unwrap().branch, "baz");
        assert_eq!(
            parsed_commits[1].target.as_ref().unwrap().parent_branch,
            Some("bar".to_string())
        );

        assert_eq!(parsed_commits[2].target.as_ref().unwrap().branch, "bam");
        assert_eq!(
            parsed_commits[2].target.as_ref().unwrap().parent_branch,
            Some("baz".to_string())
        ); // Implicit parent

        // Step 2: Convert to EnhancedCommits with Notes (simulating save_note behavior)
        let enhanced_commits: Vec<EnhancedCommit<Note>> = parsed_commits
            .iter()
            .map(|commit| {
                let note = Some(Note {
                    push: commit.target.as_ref().map(
                        |Target {
                             origin,
                             branch,
                             parent_branch,
    ...
    fn test_mixed_explicit_and_implicit_parents() {
        ...
        assert_eq!(
            parsed_commits[3].target.as_ref().unwrap().parent_branch,
